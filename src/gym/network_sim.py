...
# - Added customization of reward function and network paramters
# - Customized plotting
...
class SimulatedNetworkEnv(gym.Env):
    ...
    def __init__(self,
        ...
        log.info(f"Using Bandwidth: {self.min_bw} - {self.max_bw}, Latency: {self.min_lat} - {self.max_lat}, Queue: {self.min_queue} - {self.max_queue}, Loss: {self.min_loss} - {self.max_loss}")
    ...
    def dump_events_to_file(self, filename):
        ...
        if not os.path.exists(f"logs/{START_TIME}"): #TODO: Add foldername for each experiment. Give name based on params (ALGO_NAME, ...)
...
def plot_eval(data, title, include_init=True):
    ...
    # loss_axis = latency_axis.twinx() # Create a twin y axis. #TODO
